...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-8">
        ...
        <form action="{{ route('customer.transfers.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea name="message" id="message" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                    placeholder="Add a message for the recipient...">{{ old('message') }}</textarea>
                @error('message')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('customer.transfers.sent') }}" class="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-medium transition-colors">
                    Cancel
                </a>
            </div>
        </form>
