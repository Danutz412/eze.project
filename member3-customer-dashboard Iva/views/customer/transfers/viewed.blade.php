...
@section('title', 'Viewed Items')
@section('breadcrumb', 'Viewed Items')
    ...
    <div class="p-6 border-b border-gray-200">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-3">
                <form method="GET" action="{{ route('customer.transfers.viewed') }}" class="flex items-center space-x-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-600">
                                @if($transfer->sender_ezepost_user_id == auth()->user()->ezepostUser?->id)
                                    Sent to: <span class="font-medium">{{ $transfer->receiverUser->displayname ?? 'Unknown' }}</span>
                                @else
                                    From: <span class="font-medium">{{ $transfer->senderUser->displayname ?? 'Unknown' }}</span>
                                @endif
                            </p>
                            <p class="text-xs text-gray-500 mt-1">
                                Viewed: {{ $transfer->viewed_at ? $transfer->viewed_at->diffForHumans() : 'Recently' }}
                            </p>
                        </div>
                    </div>
